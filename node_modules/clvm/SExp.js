...
function convert_atom_to_bytes(v) {
    if (__type_compatibility__1.isBytes(v)) {
        ...
        return casts_1.int_to_bytes(v, { signed: true });
    }
    else if (typeof v === "boolean") { // Tips. In Python, isinstance(True, int) == True. 
        return casts_1.int_to_bytes(v ? 1 : 0, { signed: true });
    }
    else if (typeof v === "bigint") {
        return casts_1.bigint_to_bytes(v, { signed: true });
    }
...
function to_sexp_type(value) {
    ...
    while (ops.length) {
        ...
        if (op === op_set_left) { // set left
            stack[targetIndex] = new CLVMObject_1.CLVMObject(__type_compatibility__1.t(new CLVMObject_1.CLVMObject(stack.pop()), stack[targetIndex].pair[1]));
        }
        else if (op === op_set_right) { // set right
            stack[targetIndex] = new CLVMObject_1.CLVMObject(__type_compatibility__1.t(stack[targetIndex].pair[0], new CLVMObject_1.CLVMObject(stack.pop())));
        }
...
class SExp {
    constructor(v) {
        this._atom = __python_types__1.None;
        this._pair = __python_types__1.None;
        this._atom = v.atom;
        this._pair = v.pair;
    }
    get atom() {
    ...
    nullp() {
        return this.atom !== __python_types__1.None && this.atom.length === 0;
    }
    as_int() {
        return casts_1.int_from_bytes(this.atom, { signed: true });
    }
    as_bigint() {
        return casts_1.bigint_from_bytes(this.atom, { signed: true });
    }
    as_bin() {
