...
(0, _tape.default)('name_decoding', function (t) {
  ...
  }, /Cannot decode name \(bad label\)$/); // Ensure there's enough buffer to read
  ...
  }, /Cannot decode name \(buffer overflow\)$/); // Allow only pointers backwards
  ...
  }, /Cannot decode name \(bad pointer\)$/); // A name can be only 253 characters (when connected with dots)
  ...
  }, /Cannot decode name \(name too long\)$/); // Ensure jumps don't reset the total length counter
  ...
  }, /Cannot decode name \(name too long\)$/); // Ensure a jump to a null byte doesn't add extra dots

  t.ok(packet.name.decode(Buffer.from([0x00, 0x01, 0x61, 0xc0, 0x00]), 1) === 'a'); // Ensure deeply nested pointers don't cause "Maximum call stack size exceeded" errors
...
(0, _tape.test)('buffer utf8', function (sub) {
  ['', // empty
  'basic: hi', 'japanese: 日本語', 'mixed: 日本語 hi', 'min 1 byte: \x00', 'odd 1 byte: \x4c', 'max 1 byte: \x7f', 'min 2 byte: \x80', 'odd 2 byte: \xd941', 'max 2 byte: \x7fff', 'min 3 byte: \x8000', 'odd 3 byte: \xa158', 'max 3 byte: \xffff', `4 byte: ${String.fromCodePoint(100000)}`].forEach((fixture, index) => {
    sub.test(`fixture #${index}`, t => {
      ...
      t.equals((0, _buffer_utils.toHex)(buf, 0, len), check.toString('hex'), `write: ${fixture}`);
