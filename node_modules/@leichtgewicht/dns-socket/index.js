...
class DNSSocket extends _events.EventEmitter {
  constructor(opts = {}) {
    super();
    ...
    this.socket = opts.socket || _dgram.createSocket('udp4');
  ...
  _shouldRedirect(q, result) {
    ...
    q.firstTry = Date.now();
    q.tries = 0;
    q.buffer = _buffer.Buffer.alloc(packet.encodingLength(q.query));
  ...
  response(query, response, port, host) {
    ...
    const buffer = _buffer.Buffer.alloc(packet.encodingLength(response));
  ...
  query(query, port, host, cb) {
    ...
    const buffer = _buffer.Buffer.alloc(packet.encodingLength(query));
