"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.SExpUtil = void 0;
const clvm_1 = require("clvm");
const __1 = require("..");
const condition_opcodes_1 = require("./condition_opcodes");
const condition_with_args_1 = require("./condition_with_args");
class SExpUtil {
    constructor() {
        this.MAX_BLOCK_COST_CLVM = 11000000000;
        // https://github.com/Chia-Network/chia-blockchain/blob/main/chia/wallet/puzzles/sha256tree_module.clvm.hex
        this.SHA256TREE_MODULE_PROGRAM = this.fromHex("ff02ffff01ff02ff02ffff04ff02ffff04ff05ff80808080ffff04ffff01ff02ffff03ffff07ff0580ffff01ff0bffff0102ffff02ff02ffff04ff02ffff04ff09ff80808080ffff02ff02ffff04ff02ffff04ff0dff8080808080ffff01ff0bffff0101ff058080ff0180ff018080");
        /*
        Uses chialisp to curry arguments into program (is that how you say it?)
        (venv) yakuhito@catstation:~/projects/clvm_tools$ cat curry.clvm
        ; https://chialisp.com/docs/common_functions
        (mod args
            ;; utility function used by curry
            (defun fix_curry_args (items core)
                (if items
                    (qq (c (q . (unquote (f items))) (unquote (fix_curry_args (r items) core))))
                    core
                )
            )
    
            ; (curry sum (list 50 60)) => returns a function that is like (sum 50 60 ...)
            (defun curry (func list_of_args) (qq (a (q . (unquote func)) (unquote (fix_curry_args list_of_args (q . 1))))))
    
            (curry (f args) (r args))
        )
        (venv) yakuhito@catstation:~/projects/clvm_tools$ run curry.clvm
        (a (q 2 4 (c 2 (c 5 (c 7 ())))) (c (q (c (q . 2) (c (c (q . 1) 5) (c (a 6 (c 2 (c 11 (q 1)))) ()))) 2 (i 5 (q 4 (q . 4) (c (c (q . 1) 9) (c (a 6 (c 2 (c 13 (c 11 ())))) ()))) (q . 11)) 1) 1))
        (venv) yakuhito@catstation:~/projects/clvm_tools$ opc '(a (q 2 4 (c 2 (c 5 (c 7 ())))) (c (q (c (q . 2) (c (c (q . 1) 5) (c (a 6 (c 2 (c 11 (q 1)))) ()))) 2 (i 5 (q 4 (q . 4) (c (c (q . 1) 9) (c (a 6 (c 2 (c 13 (c 11 ())))) ()))) (q . 11)) 1) 1))'
        
        (venv) yakuhito@catstation:~/projects/clvm_tools$
        */
        this.CURRY_PROGRAM = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/main/chia/wallet/puzzles/p2_delegated_puzzle_or_hidden_puzzle.clvm.hex
        this.P2_DELEGATED_PUZZLE_OR_HIDDEN_PUZZLE_PROGRAM_MOD = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/5f4e39480e2312dc93a7b3609bcea576a9a758f9/chia/wallet/puzzles/p2_delegated_puzzle_or_hidden_puzzle.py#L70
        this.DEFAULT_HIDDEN_PUZZLE_PROGRAM = this.fromHex("ff0980");
        this.DEFAULT_HIDDEN_PUZZLE_HASH = "711d6c4e32c92e53179b199484cf8c897542bc57f2b22582799f9d657eec4699";
        // https://github.com/Chia-Network/chia-blockchain/blob/5f4e39480e2312dc93a7b3609bcea576a9a758f9/chia/wallet/puzzles/calculate_synthetic_public_key.clvm.hex
        this.CALCULATE_SYNTHETIC_PUBLIC_KEY_PROGRAM = this.fromHex("ff1dff02ffff1effff0bff02ff05808080");
        // https://github.com/Chia-Network/chia-blockchain/blob/main/chia/wallet/puzzles/p2_conditions.clvm.hex
        this.P2_CONDITIONS_PROGRAM = this.fromHex("ff04ffff0101ff0280");
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/cat.clvm.hex
        this.CAT_PROGRAM_MOD = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/cat.clvm.hex.sha256tree
        this.CAT_PROGRAM_MOD_HASH = this.sha256tree(this.CAT_PROGRAM_MOD);
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/genesis_by_coin_id.clvm.hex
        this.GENESIS_BY_COIN_ID_TAIL_MOD = this.fromHex("ff02ffff03ff2fffff01ff0880ffff01ff02ffff03ffff09ff2dff0280ff80ffff01ff088080ff018080ff0180");
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/genesis_by_puzzle_hash.clvm.hex
        this.GENESIS_BY_PUZZLE_HASH_TAIL_MOD = this.fromHex("ff02ffff03ff2fffff01ff0880ffff01ff02ffff03ffff09ffff0bff82013fff02ff8202bf80ff2d80ff80ffff01ff088080ff018080ff0180");
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/everything_with_signature.clvm.hex
        this.EVERYTHING_WITH_SIGNATURE_TAIL_MOD = this.fromHex("ff02ffff01ff04ffff04ff02ffff04ff05ffff04ff5fff80808080ff8080ffff04ffff0132ff018080");
        // https://github.com/Chia-Network/chia-blockchain/blob/d0de8038cd95b71fa050f79e3685c51dcf05e13e/chia/wallet/puzzles/delegated_tail.clvm.hex
        this.DELEGATED_TAIL_MOD = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/280f462071e5fe1b7883cefac73712789e22b664/chia/wallet/puzzles/singleton_top_layer_v1_1.clvm.hex
        this.SINGLETON_TOP_LAYER_v1_1_PROGRAM_MOD = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/280f462071e5fe1b7883cefac73712789e22b664/chia/wallet/puzzles/singleton_top_layer_v1_1.clvm.hex.sha256tree
        this.SINGLETON_TOP_LAYER_v1_1_PROGRAM_MOD_HASH = "7faa3253bfddd1e0decb0906b2dc6247bbc4cf608f58345d173adb63e8b47c9f";
        // https://github.com/Chia-Network/chia-blockchain/blob/280f462071e5fe1b7883cefac73712789e22b664/chia/wallet/puzzles/singleton_launcher.clvm.hex
        this.SINGLETON_LAUNCHER_PROGRAM = this.fromHex("");
        // https://github.com/Chia-Network/chia-blockchain/blob/280f462071e5fe1b7883cefac73712789e22b664/chia/wallet/puzzles/singleton_launcher.clvm.hex.sha256tree
        this.SINGLETON_LAUNCHER_PROGRAM_HASH = "eff07522495060c066f66f32acc2a77e3a3e737aca8baea4d1a64ea4cdc13da9";
        // https://github.com/Chia-Network/chia-blockchain/blob/280f462071e5fe1b7883cefac73712789e22b664/chia/wallet/puzzles/p2_singleton.clvm.hex
        this.P2_SINGLETON_PROGRAM_MOD = this.fromHex("");
    }
    fromHex(hex) {
        const s = new clvm_1.Stream(new clvm_1.Bytes(Buffer.from(hex, "hex")));
        const sexp = (0, clvm_1.sexp_from_stream)(s, clvm_1.SExp.to);
        return sexp;
    }
    toHex(sexp) {
        var _a;
        return (_a = sexp === null || sexp === void 0 ? void 0 : sexp.as_bin().hex()) !== null && _a !== void 0 ? _a : "";
    }
    bytesToAtom(data) {
        return clvm_1.SExp.to(clvm_1.Bytes.from(data, "hex"));
    }
    run(program, solution, max_cost) {
        const res = (0, clvm_1.run_program)(program, solution, clvm_1.OPERATOR_LOOKUP, max_cost)[1];
        return new clvm_1.SExp(res);
    }
    runWithCost(program, solution, max_cost) {
        const r = (0, clvm_1.run_program)(program, solution, clvm_1.OPERATOR_LOOKUP, max_cost);
        return [
            new clvm_1.SExp(r[1]),
            r[0]
        ];
    }
    sha256tree(program) {
        const result = this.run(this.SHA256TREE_MODULE_PROGRAM, clvm_1.SExp.to([program]));
        return Buffer.from(
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        result.atom.data()).toString("hex");
    }
    // returns 3 values:
    // 1: err (false if there wasno error)
    // 2: conditions dictionary
    // 3: cost
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L114
    conditionsDictForSolution(puzzleReveal, solution, maxCost) {
        const [error, result, cost] = this.conditionsForSolution(puzzleReveal, solution, maxCost);
        if (error || result === null) {
            return [true, null, 0];
        }
        return [false, this.conditionsByOpcode(result), cost];
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L125
    conditionsForSolution(puzzleReveal, solution, maxCost) {
        try {
            const [cost, r] = (0, clvm_1.run_program)(puzzleReveal, solution, clvm_1.OPERATOR_LOOKUP, maxCost);
            const [error, result] = this.parseSExpToConditions(r);
            return [error, result, cost];
        }
        catch (_) {
            return [true, null, 0];
        }
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L33
    parseSExpToConditions(sexp) {
        const results = [];
        for (const e of sexp.as_iter()) {
            const [error, cvp] = this.parseSExpToCondition(e);
            if (error || cvp === null) {
                return [true, null];
            }
            results.push(cvp);
        }
        return [false, results];
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L18
    parseSExpToCondition(sexp) {
        const as_atoms = this.asAtomList(sexp);
        if (as_atoms.length < 1) {
            return [true, null];
        }
        const opcode = as_atoms[0];
        const cwa = new condition_with_args_1.ConditionWithArgs();
        cwa.opcode = opcode;
        cwa.vars = as_atoms.slice(1);
        return [false, cwa];
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/types/blockchain_format/program.py#L104
    asAtomList(sexp) {
        const items = [];
        // eslint-disable-next-line no-constant-condition
        try {
            for (const e of sexp.as_iter()) {
                if (e.atom) {
                    items.push(e.atom.hex());
                }
                else {
                    items.push(this.toHex(clvm_1.SExp.to(e)));
                }
            }
        }
        catch (_) {
            // do nothing
        }
        return items;
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L52
    conditionsByOpcode(conditions) {
        const d = new Map();
        for (let i = 0; i < conditions.length; i++) {
            const cvp = conditions[i];
            let item = d.get(cvp.opcode);
            if (item === undefined) {
                item = [];
            }
            item.push(cvp);
            d.set(cvp.opcode, item);
        }
        return d;
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/fe77c690182e97f7ef13d1fb383481f32efe2e87/chia/util/condition_tools.py#L81
    pkmPairsForConditionsDict(conditionsDict, coinName, additionalData) {
        var _a, _b;
        const ret = [];
        for (const cwa of (_a = conditionsDict.get(condition_opcodes_1.ConditionOpcode.AGG_SIG_UNSAFE)) !== null && _a !== void 0 ? _a : []) {
            ret.push([cwa.vars[0], cwa.vars[1]]);
        }
        for (const cwa of (_b = conditionsDict.get(condition_opcodes_1.ConditionOpcode.AGG_SIG_ME)) !== null && _b !== void 0 ? _b : []) {
            ret.push([cwa.vars[0], cwa.vars[1] + coinName + additionalData]);
        }
        return ret;
    }
    curry(program, args) {
        const currySolution = clvm_1.SExp.to([
            program,
            ...args
        ]);
        return this.run(this.CURRY_PROGRAM, currySolution);
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/5f4e39480e2312dc93a7b3609bcea576a9a758f9/chia/wallet/puzzles/p2_delegated_puzzle_or_hidden_puzzle.py
    calculateSyntheticPublicKey(publicKey, hiddenPuzzleHash = this.DEFAULT_HIDDEN_PUZZLE_HASH) {
        const { G1Element } = (0, clvm_1.getBLSModule)();
        const r = this.run(this.CALCULATE_SYNTHETIC_PUBLIC_KEY_PROGRAM, clvm_1.SExp.to([
            this.bytesToAtom(__1.Util.key.publicKeyToHex(publicKey)),
            this.bytesToAtom(hiddenPuzzleHash)
        ]));
        return G1Element.from_bytes(
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        r.atom.data());
    }
    // https://github.com/Chia-Network/chia-blockchain/blob/5f4e39480e2312dc93a7b3609bcea576a9a758f9/chia/wallet/puzzles/p2_delegated_puzzle_or_hidden_puzzle.py
    standardCoinPuzzle(key, isSyntheticKey = false) {
        const syntheticPublicKey = isSyntheticKey ?
            key : this.calculateSyntheticPublicKey(key, this.DEFAULT_HIDDEN_PUZZLE_HASH);
        return this.curry(this.P2_DELEGATED_PUZZLE_OR_HIDDEN_PUZZLE_PROGRAM_MOD, [
            this.bytesToAtom(__1.Util.key.publicKeyToHex(syntheticPublicKey)),
        ]);
    }
    standardCoinSolution(conditions) {
        return clvm_1.SExp.to([
            clvm_1.SExp.to([]),
            this.run(this.P2_CONDITIONS_PROGRAM, clvm_1.SExp.to([
                clvm_1.SExp.to(conditions),
            ])),
            clvm_1.SExp.to([])
        ]);
    }
    // https://github.com/irulast/chia-crypto-utils/blob/f1d8ef4c8ed6134e392f4b42ef9a4e30d449f3d3/lib/src/clvm/program.dart#L158
    // https://github.com/Chia-Network/chia-blockchain/blob/22e47a81dfbed053c7a8044b6dc254b8b152b0ab/chia/types/blockchain_format/program.py#L113
    uncurry(program) {
        var _a;
        const programList = [];
        try {
            for (const elem of program.as_iter()) {
                programList.push(elem);
            }
            // eslint-disable-next-line no-empty
        }
        catch (_) { }
        if (programList.length !== 3) {
            // 'Program is wrong length, should contain 3: (operator, puzzle, arguments)',
            return null;
        }
        if (((_a = programList[0].atom) === null || _a === void 0 ? void 0 : _a.hex()) !== "02") {
            // 'Program is missing apply operator (a)'
            return null;
        }
        const uncurriedModule = this._matchQuotedProgram(programList[1]);
        if (uncurriedModule === null) {
            // 'Puzzle did not match expected pattern'
            return null;
        }
        const uncurriedArgs = this._matchCurriedArgs(programList[2]);
        if (uncurriedArgs.length === 0) {
            return null;
        }
        return [uncurriedModule, uncurriedArgs];
    }
    // https://github.com/irulast/chia-crypto-utils/blob/f1d8ef4c8ed6134e392f4b42ef9a4e30d449f3d3/lib/src/clvm/program.dart#L177
    _matchQuotedProgram(program) {
        var _a, _b;
        const cons = (_a = program.as_pair()) !== null && _a !== void 0 ? _a : [null, null];
        if (((_b = cons[0]) === null || _b === void 0 ? void 0 : _b.atom.hex()) === "01" && !cons[1].atom) {
            return cons[1];
        }
        return null;
    }
    // https://github.com/irulast/chia-crypto-utils/blob/f1d8ef4c8ed6134e392f4b42ef9a4e30d449f3d3/lib/src/clvm/program.dart#L185
    _matchCurriedArgs(program) {
        try {
            const result = this._matchCurriedArgsHelper([], program);
            return result;
        }
        catch (_) {
            return [];
        }
    }
    // https://github.com/irulast/chia-crypto-utils/blob/f1d8ef4c8ed6134e392f4b42ef9a4e30d449f3d3/lib/src/clvm/program.dart#L190
    _matchCurriedArgsHelper(uncurriedArguments, inputProgram) {
        const inputProgramList = [];
        if (inputProgram.atom === null) {
            for (const elem of inputProgram.as_iter()) {
                inputProgramList.push(new clvm_1.SExp(elem));
            }
        }
        // base case
        if (inputProgramList.length === 0) {
            return uncurriedArguments;
        }
        const atom = this._matchQuotedAtom(inputProgramList[1]);
        if (atom !== null) {
            uncurriedArguments.push(atom);
        }
        else {
            const program = this._matchQuotedProgram(inputProgramList[1]);
            if (program === null) {
                return uncurriedArguments;
            }
            uncurriedArguments.push(program);
        }
        const nextArgumentToParse = inputProgramList[2];
        return this._matchCurriedArgsHelper(uncurriedArguments, nextArgumentToParse);
    }
    // https://github.com/irulast/chia-crypto-utils/blob/f1d8ef4c8ed6134e392f4b42ef9a4e30d449f3d3/lib/src/clvm/program.dart#L213
    _matchQuotedAtom(program) {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        const cons = program.as_pair();
        if (cons[0].atom.hex() === "01" && cons[1].atom) {
            return cons[1];
        }
        return null;
    }
    CATPuzzle(TAILProgramHash, innerPuzzle) {
        return this.curry(this.CAT_PROGRAM_MOD, [
            this.bytesToAtom(this.CAT_PROGRAM_MOD_HASH),
            this.bytesToAtom(TAILProgramHash),
            innerPuzzle
        ]);
    }
    CATSolution(innerPuzzleSolution, lineageProof, prevCoinId, thisCoinInfo, nextCoinProof, prevSubtotal, extraDelta) {
        return clvm_1.SExp.to([
            innerPuzzleSolution,
            lineageProof !== null && lineageProof !== void 0 ? lineageProof : clvm_1.SExp.FALSE,
            this.bytesToAtom(prevCoinId),
            __1.Util.coin.toProgram(thisCoinInfo),
            __1.Util.coin.toProgram(nextCoinProof),
            this.bytesToAtom(__1.Util.coin.amountToBytes(prevSubtotal)),
            this.bytesToAtom(__1.Util.coin.amountToBytes(extraDelta)),
        ]);
    }
    genesisByCoinIdTAIL(genesisId) {
        return this.curry(this.GENESIS_BY_COIN_ID_TAIL_MOD, [
            this.bytesToAtom(genesisId)
        ]);
    }
    genesisByPuzzleHashTAIL(puzzleHash) {
        return this.curry(this.GENESIS_BY_PUZZLE_HASH_TAIL_MOD, [
            this.bytesToAtom(puzzleHash)
        ]);
    }
    everythingWithSignatureTAIL(pubKey) {
        return this.curry(this.EVERYTHING_WITH_SIGNATURE_TAIL_MOD, [
            this.bytesToAtom(pubKey)
        ]);
    }
    delegatedTAIL(pubKey) {
        return this.curry(this.DELEGATED_TAIL_MOD, [
            this.bytesToAtom(pubKey)
        ]);
    }
    singletonPuzzle(launcherId, innerPuzzle) {
        return this.curry(this.SINGLETON_TOP_LAYER_v1_1_PROGRAM_MOD, [
            clvm_1.SExp.to(new clvm_1.Tuple(// SINGLETON_STRUCT
            this.bytesToAtom(this.SINGLETON_TOP_LAYER_v1_1_PROGRAM_MOD_HASH), clvm_1.SExp.to(new clvm_1.Tuple(this.bytesToAtom(launcherId), this.bytesToAtom(this.SINGLETON_LAUNCHER_PROGRAM_HASH))))),
            innerPuzzle
        ]);
    }
    singletonSolution(lineageProof, amount, innerSolution) {
        return clvm_1.SExp.to([
            lineageProof,
            this.bytesToAtom(__1.Util.coin.amountToBytes(amount)),
            innerSolution
        ]);
    }
    singletonLauncherSolution(singletonFullPuzzleHash, amount, keyValueList) {
        const l = [];
        for (const [k, v] of keyValueList) {
            l.push(clvm_1.SExp.to([k, v]));
        }
        return clvm_1.SExp.to([
            this.bytesToAtom(singletonFullPuzzleHash),
            this.bytesToAtom(__1.Util.coin.amountToBytes(amount)),
            clvm_1.SExp.to(l)
        ]);
    }
    payToSingletonPuzzle(launcherId) {
        return this.curry(this.P2_SINGLETON_PROGRAM_MOD, [
            this.bytesToAtom(this.SINGLETON_TOP_LAYER_v1_1_PROGRAM_MOD_HASH),
            this.bytesToAtom(launcherId),
            this.bytesToAtom(this.SINGLETON_LAUNCHER_PROGRAM_HASH)
        ]);
    }
    payToSingletonSolution(singletonInnerPuzzleHash, myId) {
        return clvm_1.SExp.to([
            this.bytesToAtom(singletonInnerPuzzleHash),
            this.bytesToAtom(myId),
        ]);
    }
}
exports.SExpUtil = SExpUtil;
//# sourceMappingURL=index.js.map